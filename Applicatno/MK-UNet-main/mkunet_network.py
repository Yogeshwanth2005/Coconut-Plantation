...
class MK_UNet_ShallowDec(nn.Module):
    """
    Hybrid architecture: Full MK-UNet encoder + Shallow CNN decoder.
    - Encoder: Multi-kernel IRBs (identical to MK_UNet) for strong feature extraction.
    - Decoder: Simple Conv+BN+ReLU with concatenated skip connections.
    - No attention gates, no channel/spatial attention in decoder (only at bottleneck).
    - Designed for small datasets and region-level segmentation tasks.
    """

    def __init__(self, num_classes=15, in_channels=3, channels=[64,128,256,384,512],
                 depths=[1, 1, 1, 1, 1], kernel_sizes=[1,3,5], expansion_factor=2, enable_cls=False, **kwargs):
        super().__init__()

        # ============ ENCODER (identical to MK_UNet) ============
        self.encoder1 = mk_irb_bottleneck(in_channels, channels[0], depths[0], 1,
                        expansion_factor=expansion_factor, dw_parallel=True, add=True, kernel_sizes=kernel_sizes)
        ...
        # ============ BOTTLENECK ATTENTION (kept for focus) ============
        self.bottleneck_ca = ChannelAttention(channels[4], ratio=16)
        self.bottleneck_sa = SpatialAttention()

        # ============ SHALLOW CNN DECODER ============
        # Each stage: upsample -> concat skip -> Conv+BN+ReLU
        self.decoder1 = nn.Sequential(
            nn.Conv2d(channels[4] + channels[3], channels[3], 3, 1, 1, bias=False),
            nn.BatchNorm2d(channels[3]),
            nn.ReLU(inplace=True)
        )
        self.decoder2 = nn.Sequential(
            nn.Conv2d(channels[3] + channels[2], channels[2], 3, 1, 1, bias=False),
            nn.BatchNorm2d(channels[2]),
            nn.ReLU(inplace=True)
        )
        self.decoder3 = nn.Sequential(
            nn.Conv2d(channels[2] + channels[1], channels[1], 3, 1, 1, bias=False),
            nn.BatchNorm2d(channels[1]),
            nn.ReLU(inplace=True)
        )
        self.decoder4 = nn.Sequential(
            nn.Conv2d(channels[1] + channels[0], channels[0], 3, 1, 1, bias=False),
        ...
        # ============ CLASSIFICATION HEAD (optional, multi-label) ============
        self.enable_cls = enable_cls
        if enable_cls:
            cls_classes = max(1, num_classes - 1)  # exclude background
    ...
    def forward(self, x):
        ...
        # ---- Encoder ----
        out = F.max_pool2d(self.encoder1(x), 2, 2)
        t1 = out  # [B, ch[0], H/2, W/2]

        out = F.max_pool2d(self.encoder2(out), 2, 2)
        t2 = out  # [B, ch[1], H/4, W/4]

        out = F.max_pool2d(self.encoder3(out), 2, 2)
        t3 = out  # [B, ch[2], H/8, W/8]

        out = F.max_pool2d(self.encoder4(out), 2, 2)
        t4 = out  # [B, ch[3], H/16, W/16]

        # ---- Bottleneck ----
        out = F.max_pool2d(self.encoder5(out), 2, 2)  # [B, ch[4], H/32, W/32]
        out = self.bottleneck_ca(out) * out
        out = self.bottleneck_sa(out) * out

        # Classification branch (from bottleneck features)
        cls_logits = self.cls_head(out) if self.enable_cls else None

        # ---- Shallow Decoder ----
        # Stage 1: upsample + concat t4
        out = F.interpolate(out, scale_factor=2, mode='bilinear', align_corners=False)
        out = torch.cat([out, t4], dim=1)
        out = self.decoder1(out)

        # Stage 2: upsample + concat t3
        out = F.interpolate(out, scale_factor=2, mode='bilinear', align_corners=False)
        out = torch.cat([out, t3], dim=1)
        out = self.decoder2(out)

        # Stage 3: upsample + concat t2
        out = F.interpolate(out, scale_factor=2, mode='bilinear', align_corners=False)
        out = torch.cat([out, t2], dim=1)
        out = self.decoder3(out)

        # Stage 4: upsample + concat t1
        out = F.interpolate(out, scale_factor=2, mode='bilinear', align_corners=False)
